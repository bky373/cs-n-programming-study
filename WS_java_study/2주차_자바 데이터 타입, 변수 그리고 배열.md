...
> **박싱 및 언박싱 변환**은 본래 기본 타입 변환에 속하나, 내용이 너무 길어져 래퍼 클래스와 함께 아래 주제로 따로 빼놓았다. 
...
자바의 8가지 **기본 타입**에는 각각 **전용 클래스**가 있다. 기본 타입을, 해당 클래스의 객체로 래핑하기 때문에 이들을 가리켜 래퍼 클래스라고 한다. 래퍼 클래스는 `java.lang` 패키지의 일부이다.

![](https://images.velog.io/images/bky373/post/67e47f22-3841-4118-ac21-17a0fdb34142/image.png)

### 사용 이유

기본적으로 **제네릭 클래스는 객체(Object)에서만 작동**하며 기본 요소를 지원하지 않는다. 따라서 제네릭 클래스로 작업하려면 원시 값을 래퍼 객체로 변환해야 한다.

예를 들어, 자바 컬렉션 프레임워크(Java Collection Framework)는 오직 객체들로만 작동한다. 오래 전(Java 5 이전, 거의 15 년 전)에는 오토 박싱(auto boxing)이 없었고, Integer 컬렉션으로 예를 들면 단순한 `add(5)`조차 호출할 수 없었다. 그 당시에는 이러한 기본 값을 래퍼 클래스로 수동으로 변환하고 컬렉션에 저장해야 했다. 

오늘날에는 오토 박싱을 사용하면 `ArrayList.add(101)`을 쉽게 수행할 수 있다. 왜냐하면 내부적으로 자바가 `valueOf()` 메소드를 사용하여 ArrayList에 저장하기 전에 기본 값을 정수로 변환해주기 때문이다.
...
위의 표에 따르면, 아래와 같이 래퍼클래스 객체를 생성하고 출력할 수 있다.
...
하지만 자바 9부터는 박스형 기본 타입의 생성자 지원이 중단되었다. 실제로 `vscode`에서 사용하려고 하면, 아래처럼 노란 줄로 `Deprecated` 표시가 나타난다. 따라서 생성자 대신 두 번째 사진에서처럼 **팩토리 메소드**를 사용하는 것이 좋다.
...
`valueOf()` 메소드는 지정된 `int` 값을 나타내는 인스턴스를 반환한다. 이때, 결과 반환 값은 **캐시 값**이므로 매우 효율적이다. 항상 `-128`에서 `127` 사이의 값을 캐시하지만, 이 범위 밖의 다른 값도 캐시할 수 있다. 만약 `String`을 `Integer`로 변환해야 한다면, `String`이 래퍼 클래스가 아니기 때문에 `parseInt()` 메소드를 사용해야 한다.
...
기본 유형의 표현식을 **레퍼런스 유형의 표현식으로 변환**하는 것을 말한다. 자바에는 각각의 프리미티브 유형에 대한 **래퍼 클래스**(Wrapper Class)를 가지고 있다. 자바 5 이후로 자동으로, 프리미티브 요소에서 객체로, 또는 그 반대로 변환하는 기능이 추가되었다.

```java
// 기본 유형 int를 레퍼런스 유형 Integer로 자동 변환(자바 5 이후 가능)
...
레퍼런스 유형의 표현식을 **기본 유형의 표현식으로 변환**하는 것을 말한다. 박싱 변환과 마찬가지로, 자바 5 이후로 자동 변환 기능이 추가되었다.

```java
// 레퍼런스 유형 Integer를 기본 유형 int로 자동 변환(자바 5 이후 가능)
...
배열의 유형은 보통 `type[]`으로 작성하는데, `type`은 배열의 요소가 가지고 있는 자료형을 나타낸다. 배열 선언 방식은 아래와 같이 두 가지가 있다.
...
- 자바 10부터 기능이 추가되었다.
- 각 메소드 호출과 해당 선언을 살펴보고 호출을 적용 할 수있는 유형 인수 (또는 인수)를 결정하는 Java 컴파일러의 기능이다.
...
> Reference: https://www.notion.so/2-38b5d67c7f5a48238529bb8f1617ea0d