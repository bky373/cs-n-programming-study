...
1. [소개 및 사용 이유](#소개-및-사용-이유)
2. [제네릭 핵심 아이디어](#제네릭-핵심-아이디어)
3. [제네릭 메소드](#제네릭-메소드)
4. [타입 매개변수 제한](#타입-매개변수-제한)
5. [다중 제한](#다중-제한)
6. [와일드 카드](#와일드-카드)
7. [타입 삭제](#타입-삭제)
8. [Ref](#Ref)
...
> Generics,  **`T`** 또는 **다이아몬드 연산자(`<>`)** 를 떠올리자

생활코딩에 따르면, 제네릭(Generic)은 클래스 내부에서 사용할 데이터 **타입을 외부에서 지정**하는 기법을 의미한다. 제네릭을 사용했을 때 얻는 이점은 다음과 같다.

1. **컴파일할 때 타입을 체크**해서 **에러를 사전에 잡을 수 있다**.(아래 **핵심 아이디어** 참고)

2. **컴파일러가 타입 캐스팅**을 해주기 때문에 개발자가 편리하다(아래 **타입 삭제** 항목 참고).

3. 타입만 다르고 코드의 내용이 대부분 일치할 때, **코드의 재사용성**이 좋아진다.

같은 말을 한 번 더 풀면, 자바 제네릭은 **컴파일 시점에 타입 정확성(또는 안전성)** 을 확보해 **버그를 줄이고**, 타입을 넘나드는 추상화 계층을 더하기 위해 JDK 5.0에 도입되었다.

> Ref: [기본기를 쌓는 정아마추어 코딩블로그](https://jeong-pro.tistory.com/100 )
...
정의된 목록에는 모든 객체가 포함될 수 있기 때문에, 위의 코드처럼 명시적 캐스팅이 필요한 것이다.

하지만 캐스팅 작업은 매우 성가시다. 캐스트 연산자는 코드를 더욱 복잡하게 만든다. 그리고 개발자가 캐스트 연산자를 잘못 작성하면, 컴파일 때는 알지 못했던 타입 관련 에러(런타임 에러)를 경험할 수도 있다(제네릭은 컴파일 시점에 잘못된 타입을 알려준다).
...
**다이아몬드 연산자 `<type>`** 를 추가하여, 목록의 요소를 `Integer` 타입으로 좁혔다. 이에 따라 컴파일러는 컴파일 시점에 올바른 타입을 적용할 수 있게 되었다. 
